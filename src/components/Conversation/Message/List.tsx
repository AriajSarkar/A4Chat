...
export function MessageList({ isStreaming, messages, status }: MessageListProps) {
    ...
    return (
        <div className="relative flex min-h-0 flex-1 flex-col">
            <section
                ref={scrollRef}
                className="min-h-0 flex-1 overflow-y-auto px-4 pt-2 pb-6 md:px-8"
            ...
            <AnimatePresence>
                {showScrollBtn ? (
                    <motion.button
                        ...
                        className="bg-surface-2 text-text-secondary hover:bg-surface-3 hover:text-text-primary absolute bottom-2 left-1/2 z-30 flex h-9 -translate-x-1/2 items-center gap-1.5 rounded-full border border-white/10 px-2.5 text-[13px] font-medium shadow-lg shadow-black/40 backdrop-blur-md transition-all active:scale-95"
                        exit={{ opacity: 0, y: 10 }}
