...
export function ConversationWorkspace() {
                ...
                if (p?.length) {
                    ...
                    const entries = await Promise.all(
                        p.map(async (provider) => {
                            const cached = await loadProviderModelCache(provider.id).catch(
                                () => null,
                            );
                    ...
                    for (const provider of p) {
                        const currentCacheAt =
                            checkedAtByProvider.get(provider.id) ??
                            providerModelCheckedAt[provider.id] ??
                            0;
    ...
    const handleToggleFavorite = useCallback(
        async (providerId: string, modelId: string) => {
            ...
            void saveProviderModelCache(
                providerId,
                nextModels,
                providerModelCheckedAt[providerId] ??
                    getLatestModelSyncAt(nextModels) ??
                    Date.now(),
            );
                                ...
                                m.id === pendingId
                                    ? {
                                          ...
                                          content:
                                              ...
                                              m.content,
                                          reasoning:
                                              (response.reasoning ?? streamReasoningRef.current) ||
                                              undefined,
    ...
    return (
        <div className="bg-background text-foreground absolute inset-0 flex overflow-hidden">
            <NavigationSidebar
