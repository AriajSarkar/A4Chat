...
export function SettingsDialog({
    ...
    const [activeSection, setActiveSection] =
        useState<(typeof sections)[number]["id"]>("providers");
            ...
            {open ? (
                ...
                >
                    <motion.form
                        ...
                        className="safe-top safe-bottom bg-surface-1 flex h-dvh w-full overflow-hidden border-white/8 shadow-2xl shadow-black/40 sm:h-[min(760px,92dvh)] sm:w-[min(960px,96vw)] sm:rounded-2xl sm:border md:rounded-3xl"
                    ...
                    >
                        ...
                        <aside className="hidden w-60 shrink-0 border-r border-white/6 p-3 md:block">
                            <button
                                ...
                                className="text-text-secondary mb-4 grid size-11 place-items-center rounded-xl bg-white/6 transition-colors hover:bg-white/10"
                                onClick={onClose}
                                type="button"
                            >
                                <RiCloseLine size={22} />
                        ...
                        <section className="min-w-0 flex-1 overflow-y-auto">
                            <div className="bg-surface-1/95 sticky top-0 z-10 flex h-14 items-center justify-between border-b border-white/6 px-5 backdrop-blur md:h-16 md:px-8">
                                <h2 className="text-text-primary text-lg font-semibold md:text-xl">
                                    Settings
                                </h2>
                                <button
                                    aria-label="Close settings"
                                    className="text-text-secondary grid size-10 place-items-center rounded-xl transition-colors hover:bg-white/8 md:hidden"
                                    onClick={onClose}
                                    type="button"
                                >
                                    <RiCloseLine size={22} />
                            ...
                            <div className="bg-surface-1/95 sticky bottom-0 flex justify-end gap-3 border-t border-white/6 px-5 py-4 backdrop-blur md:px-8">
                                <button
                                    className="text-text-tertiary rounded-full px-4 py-2 text-sm transition-colors hover:bg-white/6"
                                    onClick={onClose}
                                    type="button"
                                >
                                    Cancel
                                ...
                                <button
                                    className="bg-accent rounded-full px-5 py-2 text-sm font-semibold text-white shadow-[0_0_16px_rgba(61,139,255,0.2)] transition-all hover:shadow-[0_0_24px_rgba(61,139,255,0.35)] disabled:opacity-50 disabled:shadow-none"
                                    disabled={saving}
                                    type="submit"
                                >
                                    {saving ? "Saving…" : "Save"}
