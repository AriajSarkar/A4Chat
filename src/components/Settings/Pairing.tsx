...
function QrGeneratePanel({
    ...
    return (
        <div className="bg-surface-0 rounded-2xl border border-white/6 px-4 py-5 sm:px-5">
            ...
            <p className="text-text-tertiary mb-5 text-sm leading-6">
                Generate a QR code for any provider. Scan it on another device to add the
                connection. Only the base URL is shared —{" "}
                <strong>API keys are never included</strong>.
                <br />
                Localhost addresses are converted to your machine&apos;s LAN IP when possible so
                phones can reach the server.
            </p>

            {enabledProviders.length === 0 ? (
                <p className="text-text-quaternary rounded-xl border border-dashed border-white/8 px-4 py-6 text-center text-sm">
                    ...
                    <AnimatePresence>
                        {qrValue ? (
                            ...
                            >
                                ...
                                <button
                                    className="text-text-tertiary flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs transition-colors hover:bg-white/6"
                                    onClick={() =>
                                        selectedProviderId && void generateQr(selectedProviderId)
                                    }
                                    type="button"
                                >
        ...
        <div className="bg-surface-0 rounded-2xl border border-white/6 px-4 py-5 sm:px-5">
            ...
            {scanning ? (
                        ...
                        >
                            {!scanSuccess && (
                                <motion.div
                                    ...
                                    className="bg-accent absolute right-0 left-0 h-0.5 shadow-[0_0_12px_3px_rgba(61,139,255,0.6)]"
                                />
                            )}
                        </div>
                    </div>
                    <button
                        className="absolute top-3 right-3 grid size-10 place-items-center rounded-full bg-black/60 text-white backdrop-blur transition-colors hover:bg-black/80"
                        onClick={stopCamera}
                        type="button"
                    >
                        <RiCloseLine size={20} />
                ...
                <div className="flex flex-col items-center gap-4">
                    {error ? (
                        <div className="border-danger/20 bg-danger/6 flex w-full items-start gap-2 rounded-xl border px-4 py-3 text-sm text-red-300">
                            <RiErrorWarningLine className="mt-0.5 shrink-0" size={16} />
                            <span>{error}</span>
                        </div>
                    ) : null}
                    <button
                        className="bg-accent/15 text-accent hover:bg-accent/25 flex items-center gap-2 rounded-xl px-6 py-3 text-sm font-medium transition-all active:scale-95"
                    ...
                    <p className="text-text-quaternary text-center text-xs">
